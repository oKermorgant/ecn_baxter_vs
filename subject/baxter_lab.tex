...
On the P-ro computers, it can be downloaded and compiled using \okttt{rosdl ecn_baxter_vs}\\
...
\subsection{Structure of the \okttt{ecn_baxter_vs} package}

The only file to be modified is \okttt{main.cpp}, which is the main file for the C++ program. \\
The simulation can be launched with: \okttt{roslaunch ecn_baxter_vs sim.launch}.
...
The current features are accessed through \okttt{arm.x(), arm.y(), arm.area()} while the desired ones are set to 0 for $(x,y)$ and accessed through \okttt{arm.area_d()} for the area. 
The error vector \okttt{e} has to be defined from the current and desired features.

Similarly, the interaction matrix of $\s$ should be defined in \okttt{L}. The feature Jacobian, that expresses the time-variation of $\s$ with regards to $\dot \q$ should then be computed
from \okttt{L} and \okttt{arm.cameraJacobian()}.

The desired joint velocities can then be computed with $\dot \q = -\lambda\J^+\e$. Of course this control law does not take into account the joint limits. \\The gain $\lambda$ should be equal to \okttt{arm.lambda()}, in order to be tuned online from the Baxter image window slider.
...
\left\{\begin{array}{ll}
	...
	\q^{s-} = & \q^{-} + \rho(\q^+ - \q^-)
\end{array}\right.
\end{equation*}$\rho$ can be tuned online through the slider, its value is available with \okttt{arm.rho()}.\\
...
A good choice is to use the default value (output of \okttt{arm.init()}) as desired value for joints $3\hdots7$ and keep their weight equal to 1. 
...
\begin{itemize}
\item \okttt{vpFeaturePoint} (variable \okttt{p}) represents a Cartesian 2D point as feature. It can be updated with \okttt{p.set_xyZ(x,y, 1);}. The main interest is that the corresponding $2\times 6$ interaction matrix can be retrieved by \okttt{p.interaction();}
...
%
%The main methods of the \okttt{Baxter} class can be seen in the \okttt{baxter_arm.h} file:
%\begin{itemize}
...
%  \item \okttt{void robot.getImagePoint(vpColVector \&)}: get the measured center of gravity of the sphere in the image, in normalized coordinates.
%  \item \okttt{vpColVector robot.getCamLimits()}: get the upper bounds for $(x,y)$. Lower bounds are the opposite.
%  \item \okttt{vpMatrix robot.getCamJacobian()}: get the Jacobian between the camera velocity screw in its own frame and the robot command $\dot \q$. This Jacobian is to be multiplied
...
% \end{itemize}
%\item \okttt{robot.setVelocity(vpColVector)}: send a $\dot \q$ velocity. Wheel velocity limits will be applied before actually sending it to the simulator.
%\end{itemize}
...
\begin{itemize}
 ...
 \item \okttt{double ecn::weight(s, s_act, s_max)}: Returns the weighting function defined during the lectures:
 \begin{equation*}
